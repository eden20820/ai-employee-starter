...
export default function CreateEmployee() {
  ...
  function startInterview() {
    ...
    startGenerating(async () => {
      try {
        const result = await advanceEmployeeInterview(text, [])
        applyInterviewResult(result)
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Could not start employee discovery')
      }
    })
  }
  ...
  function buildRoundAnswers(): InterviewAnswer[] {
    return questions.map((question) => {
      ...
      if (question.type === 'boolean' && typeof answer === 'string') {
        answer = answer === 'true'
      }
  ...
  function setAnswer(id: string, value: DraftAnswer) {
    setDraftAnswers((current) => ({ ...current, [id]: value }))
  }
  ...
  function renderQuestion(question: InterviewQuestion) {
    ...
    if (question.type === 'single_choice') {
      return <div>{question.options.map(option => <label key={option.value} style={{display:'block',margin:'8px 0'}}><input type="radio" name={question.id} checked={value === option.value} onChange={()=>setAnswer(question.id, option.value)}/> {option.label}</label>)}</div>
    }
    if (question.type === 'multiple_choice') {
      const selected = Array.isArray(value) ? value : []
      return <div>{question.options.map(option => <label key={option.value} style={{display:'block',margin:'8px 0'}}><input type="checkbox" checked={selected.includes(option.value)} onChange={()=>toggleMultiple(question.id, option.value)}/> {option.label}</label>)}</div>
    }
    if (question.type === 'boolean') {
      return <select value={typeof value === 'boolean' ? String(value) : ''} onChange={e=>setAnswer(question.id, e.target.value)}><option value="">Select…</option><option value="true">Yes</option><option value="false">No</option></select>
    }
    if (question.type === 'number') {
      return <div style={{display:'flex',gap:8,alignItems:'center'}}><input type="number" value={typeof value === 'number' || typeof value === 'string' ? value : ''} onChange={e=>setAnswer(question.id, e.target.value)}/>{question.unit && <span className="muted">{question.unit}</span>}</div>
    }
  ...
  return <div className="shell"><Sidebar active="create"/><main className="main create-wrap">
    ...
    {understanding && !plan && <div className="card" style={{marginTop:14}}>
      <div className="eyebrow">What I understand so far</div>
      <strong>{understanding.roleSummary}</strong>
      {understanding.knownFacts.length > 0 && <ul className="steps">{understanding.knownFacts.map(fact => <li key={fact}>{fact}</li>)}</ul>}
    </div>}

    {questions.length > 0 && !plan && <div className="card" style={{marginTop:14}}>
      <div className="eyebrow">Smart discovery</div>
      <h2>Let’s define how this employee should work</h2>
      <p className="muted">These questions were generated specifically from your description and previous answers.</p>
      <div style={{display:'grid',gap:20,marginTop:18}}>{questions.map((question, index) => <div key={question.id}>
        <div className="section-title">{index + 1}. {question.question}{question.required ? ' *' : ''}</div>
        {question.helpText && <p className="muted">{question.helpText}</p>}
        {renderQuestion(question)}
      </div>)}</div>
      <div className="actions" style={{marginTop:22}}><button className="button" disabled={isGenerating} onClick={continueInterview}>{isGenerating ? 'Analyzing answers…' : 'Continue →'}</button><button className="button secondary" disabled={isGenerating} onClick={startInterview}>Restart</button></div>
    </div>}

    {plan && <div className="plan">
      <div className="card"><div className="eyebrow">Ready for review · Employee Specification v{plan.schemaVersion}</div><h2>{plan.name}</h2><div className="muted">{plan.role}</div><p>{plan.goal}</p></div>
