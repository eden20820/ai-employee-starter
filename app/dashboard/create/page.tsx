...
export default function CreateEmployee() {
  ...
  function startInterview() {
    setError(''); setPlan(null); setQuestions([]); setUnderstanding(null); setAnswers([]); setCompletedRounds(0); setDraftAnswers({}); setUsageEvents([])
    startGenerating(async () => {
      try { applyInterviewResult(await advanceEmployeeInterview(text, [], 0)) }
  ...
  function continueInterview() {
    ...
    startGenerating(async () => {
      try {
        ...
        const roundsAfterThisAnswer = Math.min(completedRounds + 1, 3)
        const result = await advanceEmployeeInterview(text, allAnswers, roundsAfterThisAnswer)
        if (result.status !== 'error') {
          setAnswers(allAnswers)
          setCompletedRounds(roundsAfterThisAnswer)
        }
  ...
  return <div className="shell"><Sidebar active="create"/><main className="main create-wrap">
    ...
    <div className="card" style={{marginBottom:14}}><div className="section-title">Setup</div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><span className="tag">1 · Define the job</span><span className="tag">2 · Review employee plan</span><span className="tag">3 · Connect tools</span></div><p className="muted" style={{marginBottom:0,marginTop:10}}>Discovery is capped at three question rounds. Tool-specific setup comes later.</p></div>
    {demoMode && <div className="card" style={{marginBottom:14}}><strong>Preview demo mode</strong><p className="muted" style={{marginBottom:0}}>You can test discovery and plan generation. Database persistence and tool connection remain disabled until you sign in.</p></div>}
